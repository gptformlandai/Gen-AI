...
from semantic_search_lab.chunking import tokenize
...
SYNONYMS = {
    ...
    "csv": ["export", "file"],
    "mismatch": ["exception", "reconciliation"],
    "mismatched": ["exception", "reconciliation"],
    "transaction": ["payment", "finance"],
...
class HashingTfidfEmbeddingModel:
    """Small deterministic embedding model for learning retrieval mechanics.

    This is not a production semantic model. It combines token hashing, IDF
    weighting, and a tiny synonym map so exact-vs-ANN behavior can be studied
    ...
    def fit(self, texts: list[str]) -> None:
        ...
        self.default_idf = math.log((1 + total_documents) / 1) + 1.0
    ...
    def embed(self, text: str) -> list[float]:
        ...
        for token, frequency in term_frequency.items():
            ...
            vector[index] += sign * frequency * self.idf.get(token, self.default_idf)
